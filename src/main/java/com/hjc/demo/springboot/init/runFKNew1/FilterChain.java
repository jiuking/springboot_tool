...
//过滤链条
public class FilterChain implements RunFKFilter {
    //用List集合来存储过滤规则
    List<RunFKFilter> filters = new ArrayList<>();
    //用于标记规则的引用顺序
    int index=0;
    //往规则链条中添加规则
    public FilterChain addFilter(RunFKFilter f) {
        filters.add(f);
    ...
    public void doFilter(RunFKEntity runFKEntity, com.hjc.demo.springboot.init.runFKNew.FilterChain chain){
        ...
        //每添加一个过滤规则，index自增1
        RunFKFilter f=filters.get(index);
        index++;
        //根据索引值获取对应的规律规则对字符串进行处理
        f.doFilter(runFKEntity, chain);
    }
}